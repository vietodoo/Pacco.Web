...
namespace Pacco.Web.Clients
{
    public class CustomHttpClient : IHttpClient
    {
        ...
        private static StringContent GetPayload<T>(T request)
            => new StringContent(JsonSerializer.ToString(request, JsonSerializerOptions), Encoding.UTF8,
                "application/json");
        ...
        private static T Parse<T>(string content)
            => string.IsNullOrWhiteSpace(content) ? default : JsonSerializer.Parse<T>(content, JsonSerializerOptions);
    }
}